...
use egui::{AboveOrBelow, Align, RectAlign};
use egui::{
...
impl ComboBox {
	...
	pub fn popup_align(mut self, align: RectAlign) -> Self {
		self.rect_align = Some(align);
		self
	}
...
fn combo_box_dyn<'c, R>(
    ...
    let button_response = button_frame(ui, button_id, is_popup_open, Sense::click(), |ui| {
		...
		let above_or_below = if let Some(rect_align) = rect_align {
			if rect_align.parent.y() == Align::TOP { AboveOrBelow::Above } else { AboveOrBelow:: Below }
		} else {
			AboveOrBelow::Below
		};
        ...
        if ui.is_rect_visible(rect) {
            ...
            if let Some(icon) = icon {
                ...
                paint_default_icon(ui.painter(), icon_rect.expand(visuals.expansion), visuals, above_or_below);
            }
    ...
    let mut popup = Popup::menu(&button_response)
        .id(popup_id)
        .width(button_response.rect.width())
        .close_behavior(close_behavior)
        .style(popup_style);

	if let Some(rect_align) = rect_align {
		popup = popup.align(rect_align);
	}
