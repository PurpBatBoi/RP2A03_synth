...
use egui::{
    ...
    Label, NumExt, Pos2, Rangef, Rect, Response, Sense, TextStyle,
...
impl Slider<'_> {
    ...
    fn slider_ui(&mut self, ui: &Ui, response: &Response) {
		...
		if response.dragged() {
			let speed = if shift { 0.0002 } else { 0.002 };
			let delta = self.pointer_delta(response.drag_delta()) as f64 * speed;
			let size = self.range.end() - self.range.start();
			let new_value = self.get_value() + delta * size;
	...
	fn pointer_delta(&self, delta: Vec2,) -> f32 {
		match self.orientation {
			SliderOrientation::Horizontal => delta.x + self.delta_offset,
			SliderOrientation::Vertical => -(delta.y + self.delta_offset),
		}
	}
